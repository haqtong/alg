...
class Line:
    ...
    def at(self, x:float) -> Point:
        ...
        # print(tuple(bv + x * dv for bv, dv in zip(self.pbeg, self.pdlt)))
...
class CurveByLines(Line):
    ...
    def at(self, x: float) -> Point:
        ...
        print(self.lines[li].at(x_with_lines - li))#迭代的数据

        # 此处返回的其实就是分好段的离散坐标了
        return self.lines[li].at(x_with_lines - li)

class CurveByFormula(Line):
    def __init__(self, f: Callable[[float], Point]): # callable函数用于检查一个对象是否是可调用的。如果返回 True，object 仍然可能调用失败；但如果返回 False，调用对象 object 绝对不会成功。
        self.f = f

    def at(self, x:float) -> Point:
        return self.f(x)

def discretize(line:Line, n:int, xbeg:float=0.0, xend:float=1.0) -> Iterable[Point]: # 离散化
